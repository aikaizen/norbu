...
import { getAlphabetCards, ALPHABET_DECK_ID } from './alphabet'
import { getSadhanaCards, SADHANA_DECK_ID } from './sadhana-core'
import type { Deck } from '../schema'

export const STARTER_DECK_IDS = [ALPHABET_DECK_ID, SADHANA_DECK_ID] as const

export function getStarterDecks(): Deck[] {
  return [
    {
      id: ALPHABET_DECK_ID,
...
export function getStarterCards() {
  return [...getAlphabetCards(), ...getSadhanaCards()]
}
...
export async function ensureStarterData() {
  ...
  const missingDecks = getStarterDecks().filter((deck) => !existingDeckIds.has(deck.id))
...
export async function seedIfEmpty() {
  ...
  await db.transaction('rw', db.decks, db.cards, async () => {
    await db.decks.bulkAdd(getStarterDecks())
